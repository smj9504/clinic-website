...
export default function SkinBeautyPage() {
  const { subPages, menus, heroSlides } = useSiteData();
  const t = useT();
  const banner = getBannerImage(menus, "/skin-beauty", heroSlides[0]?.image);

  const items = (subPages ?? [])
    .filter((sp) => sp.parentMenuId === "m7" && !sp.isHidden)
    ...
    <>
      ...
      >
        ...
        <div className="container-default relative text-ink-inverse">
          ...
          >
            {getMenuLabel(menus, "/skin-beauty", t("skinBeauty.title"))}
          </h1>
        </div>
      </section>
        ...
        <div className="space-y-24">
          {items.map((item, i) => (
            <div key={item.id}>
              ...
              {item.fullBleedImage && (
                <div className="relative w-full aspect-[21/9] md:aspect-[3/1] mt-24 bg-bg-alt">
                  <Image
